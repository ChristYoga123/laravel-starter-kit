    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="row">
            <div class="col-xl">
                <div class="card mb-4">
                    <div class="card-body">
                        <form method="POST" action="{{ route('admin.settings.administrators.update', $admin->id) }}">
                            @method('PUT')
                            @csrf
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="role" class="form-select select-2 @error('role') is-invalid @enderror">
                                    ...
                                    @foreach ($roles as $role)
                                        <option {{ $role->name === $admin->roles->first()->name ? 'selected' : '' }}
                                            value="{{ $role->name }}">
                                            {{ $role->name }}</option>
                                    @endforeach
                                </select>
...
@push('scripts')
    @if ($errors->any())
        <script>
            Swal.fire(
                'Error!',
                'There are errors in your form, please check your input',
                'error'
            )
        </script>
